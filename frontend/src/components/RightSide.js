    ...
    return (
        <div className={classes.root}>
            <Grid
                ...
                {user.users.map((user) => {
                    return (
                        <Button className={classes.button} key={user.id} startIcon={<Avatar alt="avatar" className={classes.small} src={user.avatar} />} href={`/profile/${user.id}`}>{user.user_name}</Button>
                    );
                })}
            </Grid>
        </div>
